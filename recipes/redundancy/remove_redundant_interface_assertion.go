...
func (v *removeRedundantInterfaceAssertionVisitor) VisitTypeAssertion(ta *golang.TypeAssertion, p any) java.J {
	...
	// Replace the type assertion with just the asserted-on expression, which
	// already carries the correct prefix (the space between the preceding token
	// and the expression, e.g. the space after "=" in "_ = x.(any)"). We prepend
	// ta.Prefix in case the assertion itself had leading space.
